...
description: Draft, formally publish, and verify a Hugo article in this blog repository. Use when the user asks to draft, outline, write, revise, or review a product-introduction article, or to publish, formally publish, make live, deploy, or 上线/正式发布 an article under content/posts/. Covers product-article drafting structure, publication preflight, frontmatter publication date and draft state, Hugo validation, a scoped Git commit, pushing main, waiting for the matching GitHub Pages deployment, live URL verification, and optional explicitly authorized syndication to Juejin or X.
---
...
## Draft Product Introduction Articles

Apply this structure while creating the initial outline and body of an article
that introduces a product. Do not defer it to publication preflight or treat it
as a cleanup step.

1. Make the first level-two section exactly `## 快速开始`. Introductory
   paragraphs may appear before it, but no other section may precede it.
2. Draft this section before the remaining sections as the concise shortest
   path to using the product.
3. Prefer one fenced `bash` or `sh` block with only the comments needed to
   explain a prerequisite or effect.
4. Include only commands that work when copied and pasted as written. Do not
   use pseudocode, ellipses, unresolved placeholders, or fragments that the
   reader must assemble. Put required setup in the block or in one adjacent
   sentence, and verify the commands before adding them to the draft.
5. If the product has no command-line interface, provide the shortest equally
   immediate usable interaction under the same heading.
