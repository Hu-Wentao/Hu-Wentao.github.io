...
draft: true
summary: "把半结构化 Markdown 的语义理解编译成声明式查询协议，让 AI 精确定位记录，同时容忍手工编辑造成的不完整与结构漂移"
...
为此，我实现了 [make-markdown-queryable](https://github.com/Hu-Wentao/skills/tree/main/skills/make-markdown-queryable) Skill：让 AI 首次读取文档时理解它的业务结构，把理解结果写成一份很小的声明式查询协议；后续查询由确定性解析器执行，不再让模型每次重新猜测文档结构。

它的核心可以概括成一句话：

> 将一次昂贵、具有不确定性的语义理解，编译成可以重复执行和验证的查询规则。
...
人工维护的 Markdown
        ↓ 首次分析
header 中的 mdq profile ──→ 容错解析器 ──→ 带来源范围的 JSON 结果
        ↑                         ↓
        └──── 文档仍是事实源 ← 可选 sidecar 索引
~~~

- **Markdown 正文**仍然是唯一事实源，人可以继续直接编辑。
- **mdq profile** 描述记录边界、唯一键和字段来源。
- **容错解析器**按照 profile 查询当前正文，返回字段、置信度和原文位置。
- **sidecar 索引**只是可丢弃的缓存，不能覆盖当前文档。

header 里保存的不是 Shell 或 Python 脚本，而是声明式数据。文档不能要求 Agent 执行任意代码，只能告诉受信任的查询引擎“如何识别记录”。这既便于审查，也避免文档变成代码执行入口。
...
Skill 会先执行 `inspect`，观察标题层级、常见 ID 形态、重复标签、代码围栏和已有 frontmatter，再由 AI 根据多条真实记录生成一个尽可能小的 profile：
...
准备完成后，Agent 不必把全文重新放入上下文，而是调用 Skill 中的查询脚本：
...
把 profile 写入 header 不是一次性的格式转换，而是为文档增加一个持续维护的查询接口。完整生命周期包括：
...
进入包含 Markdown 文档的项目后，让 Codex 完成首次改造：
...
Skill 会引导 Agent 先检查文档结构，再最小化修改 header，执行验证和代表性查询。它不会为了获得漂亮的结构而重排整篇 Markdown。

## 仍然存在的边界

这个方案并不能自动理解任意文档。

- 首次准备仍需扫描全文，并让 AI 阅读足够有代表性的片段；结构高度不规则时，可能需要完整理解一次。语义成本从“每次查询”移动到“首次编译和规则变更时”。
...
Make Markdown Queryable 的思路，是在不牺牲 Markdown 写作自由的前提下，为文档增加一个小型、版本化、可验证的查询契约：人继续维护不完美的文档，确定性程序负责提取已有证据，AI 只在真正需要语义判断时介入。
