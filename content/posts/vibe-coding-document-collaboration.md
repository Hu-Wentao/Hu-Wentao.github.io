...
其中，\(T_{\mathrm{handoff},x}(M_r)\) 是发送消息、切换上下文、重新定位修改对象等交接成本；\(T_{\mathrm{concurrency},x}(M_r)\) 是相对完全串行基线节省的关键路径时间，包含两类收益：开发者编辑与 AI 分析互相重叠，以及不同 SPEC 的 AI 分析任务彼此并行。并发没有减少实际工作量，只是让一部分工作不再依次落在关键路径上。
