...
存储结构化数据，csv/json/jsonl/sqlite/parquet 等数据格式都已经十分成熟且方便。而在AI时代，半结构化，甚至无固定结构的Markdown文件往往更实用。需求文档可以混合标题、列表、长段落、代码示例和临时备注；尚未写完的需求可以缺少状态或详情；开发者也可以随手调整标题层级，而不必先通过 Schema 校验。这种自由非常适合人类写作，却让 AI 的精确查询变得困难。
